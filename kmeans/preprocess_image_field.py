...
UpperUpperRed = np.array([170, 45, 235])
...
BrightRedUpper = np.array([12, 70, 255])
...
# solve the derived quadratic equation
...
# create a function for drawing the KMEANS_PP_CENTERS
def draw_canvas(centers):
    # draw a blank canvas that can fit 6 circles
...
def preprocess_watershed(image, pixel_metric):
    ...
    color_canvas = draw_canvas(center)
    ...
    result = np.ones(clustered_image.shape[0:2], dtype="uint8")*255 # create a blank canvas
    valid_masks = []
    ...
    if not valid_masks:
        ...
        #cv2.imshow("Final Result Mask", final_result)
        # apply some morphological operations to the mask
        ...
        #second_eroded = cv2.erode(dilated.copy(), None, iterations=3)
        #cv2.imshow("Second Eroded Image", second_eroded)
        thresh = dilated.copy()
    ...
    #print("dist_transform", dist_transform)
    ...
    if (len(np.unique(labels)) - 1) >= 1:
        for label in np.unique(labels):
            ...
            # it on the mask
            mask = np.zeros(eroded.shape, dtype="uint8")
