...
class YieldMonitor:
   ...
   ### camera Functions
    def capture_image(self, write=True, ramp_frames = 20):
        ...
        # let the camera stabilize for 20 frames
    ...
    def calibrate_monitor(self):
        ...
        try:
            ...
            if self.pixel_metric:
                ...
                # set a default pixel metric value if the calibration fails
    ...
    def find_onions(self, write=True):
        ...
        self.small_count = self.onions[0]
        self.medium_count = self.onions[1]
        self.large_count = self.onions[2]
    ...
    def run(self):
        ...
        self.data = []
        headers = ['Small Onions','Medium Onions', 'Large Onions', 'Latitude', 'Longitude', 'Speed (km/h)', 'Time', 'Filename']
        ...
        with open(self.output_filename, "a", newline ='') as csvfile:
            ...
            while (True):
                try:
                    ...
                    self.log = [small, medium, large, lat, long, speed, gps_time]
                    cv2.imshow("result", self.result_image)
                    cv2.waitKey(500)

                    if self.result_image is not None:
                        self.log = [small, medium, large, lat, long, speed, gps_time, self.result_filename]
