...
class YieldMonitor:
   ...
   ### camera Functions
    ...
    def find_onions(self, write=True):
       ...
       if write == True:
           self.date = datetime.strftime(datetime.now(), "%Y%m%d"+"_"+"%H%M%S")
           self.result_filename = self.result_directory + self.date + ".png"
    ...
    def run(self):
        ...
        self.test_filename = self.conf["log_file_path"] + self.current_vegetable + "_" + "lot" + self.current_lot + self.date + ".csv"
        self.pretty_print("[INFO] RUNNING", "Running yield monitoring program. Press CTRL+C to exit.")
        self.pretty_print("[INFO] SAVING", "Saving results from test run as %s " % (self.test_filename))
        ...
        with open(self.test_filename, "a") as logfile:
            while (True):
                try:
